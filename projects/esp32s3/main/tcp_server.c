    ...
    void tcp_server_task(void *pvParameters)
    {
        ...
        while (1) {
            ...
            while (tcpserver_run)
            {
                ...
                if (listen_sock < 0)
                {
                    ESP_LOGI(TAG,"Failed to create socket: errno %d\r\n", errno);
                    break;
                }
                ...
                if (err != 0)
                {
                    ESP_LOGI(TAG,"Socket unable to bind: errno %d\r\n", errno);
                    break;
                }
                ...
                if (err != 0)
                {
                    ESP_LOGI(TAG,"Error occured during listen: errno %d\r\n", errno);
                    break;
                }
                ...
                while (tcpserver_run)
                {
                    ...
                    if (kSock < 0)
                    {
                        ESP_LOGI(TAG,"Unable to accept connection: errno %d\r\n", errno);
                        break;
                    }
