...
int attach(uint8_t *buffer, uint32_t length)
{
    ...
    default:
        ESP_LOGI(TAG,"attach Unknown command: %d\r\n", command);
        break;
    }
    return 0;
}
...
static void handle_device_list(uint8_t *buffer, uint32_t length)
{
    ESP_LOGI(TAG,"Handling dev list request...\r\n");
...
static void handle_device_attach(uint8_t *buffer, uint32_t length)
{
    ESP_LOGI(TAG,"Handling dev attach request...\r\n");

    //char bus[USBIP_BUSID_SIZE];
    if (length < sizeof(USBIP_BUSID_SIZE))
    {
        ESP_LOGI(TAG,"handle device attach failed!\r\n");
        return;
    }
...
static void send_device_list()
{
    ESP_LOGI(TAG,"Sending device list...\r\n");

    // send device list size:
    ESP_LOGI(TAG,"Sending device list size...\r\n");
...
static void send_interface_info()
{
    ESP_LOGI(TAG,"Sending interface info...\r\n");
...
static int usbip_urb_process(uint8_t *base, uint32_t length)
{
    ...
    while (1) {
        ...
        if (likely(command == USBIP_STAGE2_REQ_SUBMIT)) {
            ...
            ESP_LOGI(TAG,"emulate unknown command:%d\r\n", command);
            return -1;
        }
    }
...
out:
    if (ret < 0)
        ESP_LOGI(TAG,"recv failed: errno %d\r\n", errno);
    return ret;
}
